...
// quizData.ts
export const getQuizQuestions = (moduleId: number, setId: number): Question[] => {
  // Convert moduleId (number) to moduleName (string)
  const moduleName = moduleNames[moduleId - 1]; // moduleId starts from 1, array index starts from 0
  ...
  const videos = videoMap[moduleName];
